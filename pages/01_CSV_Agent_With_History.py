        ...
        if tool_name == "python_repl_tool":
            ...
            if query:
                ...
                if df_in_result is not None:
                    ...
                    print(st.session_state["messages"][-1][1][0])
...
def get_last_message_type():
    if "messages" in st.session_state and len(st.session_state["messages"]) > 0:
        return st.session_state["messages"][-1][1][-1][0]
    else:
...
def ask(query):
    ...
    if "agent" in st.session_state:
        ...
        with st.chat_message("assistant"):
            ...
            if get_last_message_type() != MessageType.DATAFRAME:
                st.write(ai_answer)
...
if apply_btn and uploaded_file:
    loaded_data = pd.read_csv(uploaded_file, encoding="utf-8")  # CSV 파일 로드
